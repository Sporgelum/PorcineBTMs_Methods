...
def ensure_mine_diagnostics_plot(study_name: str, output_dir: str) -> bool:
    """Create the two-pane diagnostics PNG from TSVs if missing.

    Returns True when a plot exists after this call, False otherwise.
    """
    diag_dir = os.path.join(output_dir, "mine_diagnostics")
    batch_path = os.path.join(diag_dir, f"mine_batch_summary_{study_name}.tsv")
    loss_path = os.path.join(diag_dir, f"mine_loss_curve_{study_name}.tsv")
    plot_path = os.path.join(diag_dir, f"mine_diagnostics_plot_{study_name}.png")

    if os.path.exists(plot_path):
        return True
    if not (os.path.exists(batch_path) and os.path.exists(loss_path)):
        return False

    try:
        batch_df = pd.read_csv(batch_path, sep="\t")
        loss_df = pd.read_csv(loss_path, sep="\t")
        if len(batch_df) == 0 or len(loss_df) == 0:
            return False
        out = _save_mine_diag_plot(batch_df, loss_df, study_name, diag_dir)
        print(f"[SAVED] {out}")
        return True
    except Exception as e:
        print(f"[WARN] Could not build diagnostics plot for {study_name} from TSVs: {e}")
        return False
...
def save_mine_diagnostics(
    ...
    for d in diagnostics:
        lc = d["loss_curve"]
        mi_train_curve = d.get("mi_curve_train", [])
        rows.append({
            "batch_id": d["batch_id"],
            "n_pairs": d["n_pairs"],
            ...
            "initial_train_mi": mi_train_curve[0] if mi_train_curve else float("nan"),
            "final_train_mi": mi_train_curve[-1] if mi_train_curve else float("nan"),
    ...
    if max_epochs > 0:
        ...
        epoch_train_mi = np.full((len(diagnostics), max_epochs), np.nan)
        for i, d in enumerate(diagnostics):
            lc = d["loss_curve"]
            epoch_losses[i, :len(lc)] = lc
            mc = d.get("mi_curve_train", [])
            if mc:
                epoch_train_mi[i, :len(mc)] = mc
        mean_loss = np.nanmean(epoch_losses, axis=0)
        std_loss = np.nanstd(epoch_losses, axis=0)
        mean_train_mi = np.nanmean(epoch_train_mi, axis=0)
        std_train_mi = np.nanstd(epoch_train_mi, axis=0)

        lc_df = pd.DataFrame({
            "epoch": np.arange(max_epochs),
            "mean_loss": mean_loss,
            "std_loss": std_loss,
            "mean_train_mi": mean_train_mi,
            "std_train_mi": std_train_mi,
        })
    ...
    # Two-pane visual summary: per-batch summary + epoch loss/MI curve
    try:
        if len(df) > 0 and max_epochs > 0:
            plot_path = _save_mine_diag_plot(df, lc_df, study_name, diag_dir)
            print(f"[SAVED] {plot_path}")
    except Exception as e:
        print(f"[WARN] Could not save MINE diagnostics plot for {study_name}: {e}")
