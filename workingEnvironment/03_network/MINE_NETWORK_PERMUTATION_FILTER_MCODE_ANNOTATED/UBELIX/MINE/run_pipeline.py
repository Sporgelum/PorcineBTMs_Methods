...
def main():
    ...
    parser.add_argument("--study-gpu-devices", type=str, nargs="*", default=[],
                        help="Optional explicit CUDA devices for study workers,"
                            " e.g. cuda:0 cuda:1.")
    parser.add_argument("--no-resume-studies", action="store_true",
                        help="Do not skip studies that already have completed outputs"
                             " in the output directory.")
    parser.add_argument("--no-reuse-mine-scores", action="store_true",
                        help="Disable reuse of cached MINE scores"
                             " (mine_diagnostics/mine_scores_{study}.npz).")
    parser.add_argument("--no-save-mine-cache", action="store_true",
                        help="Do not save per-study MINE score cache files"
    ...
    parser.add_argument("--batch-pairs", type=str, default="512",
                        help="Gene pairs per MINE batch (default: 512). "
                             "Set to 'auto' to detect optimal batch size from device VRAM.")
    parser.add_argument("--mixed-precision", action="store_true",
                        help="Enable float16 forward pass for additional memory savings (experimental).")
    ...
    parser.add_argument("--min-studies-fraction", type=float, default=None,
                        help="Optional fraction of studies required for master edge "
                             "(overrides --min-studies when set, e.g. 0.3).")
    ...
    cfg.reuse_mine_scores = not args.no_reuse_mine_scores
    cfg.save_mine_score_cache = not args.no_save_mine_cache

    # MINE
    cfg.mine.hidden_dim = args.hidden
    cfg.mine.n_epochs = args.epochs
    # Handle batch_pairs as either int or "auto"
    if args.batch_pairs.lower() == "auto":
        cfg.mine.batch_pairs = "auto"
    else:
        try:
            cfg.mine.batch_pairs = int(args.batch_pairs)
        except ValueError:
            print(f"[ERROR] --batch-pairs must be an int or 'auto', got: {args.batch_pairs}")
            sys.exit(1)
    cfg.mine.mixed_precision = args.mixed_precision
    ...
    cfg.network.min_study_fraction = args.min_studies_fraction
