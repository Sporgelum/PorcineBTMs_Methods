...
def build_global_null(
    ...
    Algorithm (Section 4c):
        For each of ``n_permutations`` trials:
          ...
          3. Estimate MI(gene_i, permuted(gene_j)) via MINE.
    ...
    torch_gen = _make_torch_generator(device, seed)
    ...
    if verbose:
        print(f"  Building global MINE null: {n_permutations} permutations "
    ...
    for b in range(n_batches):
        ...
        # Permute gene j → null: MI under independence
        perm = torch.argsort(
            torch.rand(B, n_samples, device=device, generator=torch_gen),
            dim=1,
        )
...
def build_per_pair_null(
    ...
    For each pair (g_i, g_j) and each permutation b:
      ...
      2. Estimate MI(g_i, permuted(g_j)).
    ...
    # For each permutation round, process all pairs in batches
    for p_idx in range(n_permutations):
        ...
        for b in range(n_batches):
            ...
            gi = expr_t[pair_indices[s:e, 0]]
            gj = expr_t[pair_indices[s:e, 1]]

            # Permute each gene j independently
            perm = torch.argsort(
                torch.rand(B, n_samples, device=device, generator=torch_gen),
                dim=1,
            )
